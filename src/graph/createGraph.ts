...
export function createGraph(machine: StateMachine<any, any, any>) {
	...
	const callExpression = nodeFound.getParentIfKindOrThrow(
		SyntaxKind.CallExpression,
	);
	if (!callExpression)
		throw new Error("Call expression not found at the specified location");

	const machineNode = findMachineNode(callExpression);

	const machineStatesArg = machineNode.getArguments()[0];
	if (!ObjectLiteralExpression.isObjectLiteralExpression(machineStatesArg))
		throw new Error("Machine states argument is not an object literal");
	...
	stateDeclarations.forEach(({ stateName, initializer }) => {
		...
		if (!nextStatesType)
			throw new Error(
				`Unable to determine next states type for state: ${stateName}`,
			);

		const nextStatesTypes = nextStatesType.isUnion()
			? nextStatesType.getUnionTypes()
			: [nextStatesType];

		nextStatesTypes.forEach((t) => {
			const text = t.getText();
			const stateType = text.startsWith("Guard")
				? "guard"
			...
			if (!stateType)
				throw new Error(
					`Unsupported next state type: ${text} in state: ${stateName}`,
				);

			const nextStatesArgs = t.getAliasTypeArguments();
			if (nextStatesArgs.length !== 1)
				throw new Error(
					`Unexpected number of type arguments for next state type: ${text} in state: ${stateName}`,
				);

			const nextStatesArg = nextStatesArgs[0]!;
			const nextStates: string[] = [];

			if (nextStatesArg.isUnion())
				nextStatesArg.getUnionTypes().forEach((ut) => {
					if (ut.isStringLiteral())
						nextStates.push(ut.getLiteralValue() as string);
					else
						throw new Error(
							`Unsupported union type in next states: ${ut.getText()} in state: ${stateName}`,
						);
				});
			else if (nextStatesArg.isStringLiteral())
				nextStates.push(nextStatesArg.getLiteralValue() as string);
			else
				throw new Error(
					`Unsupported next states argument type: ${nextStatesArg.getText()} in state: ${stateName}`,
				);
	...
	graph.setInitialNode(
		// @ts-expect-error private field access
		machine.initialState,
	);
	graph.setFinalNode("$_END");
...
function getCallerFile() {
	...
	const callerSite = callSites.find(
		(site) =>
			site.scriptName !== fileName && site.scriptName !== `file://${fileName}`,
	);
...
function findCallerFunctionNode(
	...
	if (!nodeFound)
		throw new Error("No matching node found at the specified location");
...
function findMachineNode(callExpression: CallExpression) {
	...
	if (Identifier.isIdentifier(machineArg)) {
		...
		if (definitions.length > 1)
			throw new Error("Multiple definitions found for the machine argument");
		if (definitions.length === 0)
			throw new Error("No definition found for the machine argument");

		const defNode = definitions[0];
		if (!VariableDeclaration.isVariableDeclaration(defNode))
			throw new Error(
				"Machine argument definition is not a variable declaration",
			);
		...
		if (!CallExpression.isCallExpression(initializer))
			throw new Error("Machine variable initializer is not a call expression");
...
function getValueSymbolFromProp(prop: Node) {
	...
	if (Node.isPropertyAssignment(prop)) {
		...
		if (Node.isPropertyAccessExpression(init))
			return init.getNameNode().getSymbol();
...
function findStateDefinitions(machineStatesArg: Node) {
	const statesObject = machineStatesArg.asKindOrThrow(
	...
	return statesObject.getProperties().flatMap((prop) => {
		...
		if (decls.length === 0)
			throw new Error(
				`No real declarations resolved for state: ${prop.getText()}`,
			);
		if (decls.length > 1)
			throw new Error(
				`Multiple declarations resolved for state: ${prop.getText()}`,
			);

		const decl = decls[0]!;
		if (!VariableDeclaration.isVariableDeclaration(decl))
			throw new Error(
				`State declaration is not a variable declaration: ${decl.getText()}`,
			);

		const initializer = decl.getInitializer();
		if (!initializer)
			throw new Error(`State variable has no initializer: ${decl.getText()}`);
		...
		if (
			...
			throw new Error(
				`State variable initializer is not a function expression: ${initializer.getText()}`,
			);

		const propName = getNameFromProp(prop);
		if (!propName)
			throw new Error(
				`Unable to determine property name for state: ${prop.getText()}`,
			);
